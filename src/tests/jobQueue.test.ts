...
async function runTests() {
  ...
  await test.run("Concurrency limit enforcement", async () => {
    ...
    const concurrencyTrackingJob = async (delay: number, jobId: number) => {
      ...
      test.log(
        `Job ${jobId} started - Current concurrent: ${currentConcurrent}`
      );
      await wait(delay);
      currentConcurrent--;
      test.log(
        `Job ${jobId} finished - Current concurrent: ${currentConcurrent}`
      );
      return delay;
    };
    ...
    test.assert(
      currentConcurrent === 0,
      `Final concurrent jobs should be 0, got ${currentConcurrent}`
    );
    test.assert(
      queue.active() === 0,
      `Final active count should be 0, got ${queue.active()}`
    );
